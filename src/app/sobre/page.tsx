...
export default function SobrePage() {
  ...
  const heroDesktop = useMedia('sobre_hero_desktop');
  const heroMobile = useMedia('sobre_hero_mobile');
  const parceriaImage = useMedia('sobre_parceria_image');
    ...
    <div className="flex flex-col min-h-screen" style={{ background: '#1A0F14' }}>
      ...
      <section className="relative min-h-[100dvh] flex flex-col justify-center overflow-hidden border-b border-[#3D2620]">
        <div className="absolute inset-0 z-0">
          <Image 
            src={heroDesktop} 
            alt={t('Sobre a Maeum Global Desktop')} 
            fill 
          ...
          <Image 
            src={heroMobile} 
            alt={t('Sobre a Maeum Global Mobile')}
            fill 
        ...
        <div className="relative z-10 px-6 sm:px-12 max-w-7xl mx-auto w-full flex flex-col items-start gap-6 pt-24">
          ...
          <div className="mt-4">
            <Link href={lp('/coreia-do-sul/jornadas')} className="flex items-center justify-center gap-3 bg-transparent border border-[#C8A27C] text-[#C8A27C] hover:bg-[#C8A27C] hover:text-[#0F0A08] font-bold text-[10px] py-4 px-8 rounded-none transition-all group uppercase tracking-widest">
      ...
      <section className="py-20 px-4 md:px-8 border-t border-[#3A232E]" style={{ background: '#1A0F14' }}>
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <span className="text-xs uppercase tracking-widest text-[#C8A27C] font-bold">{t('NOSSOS VALORES')}</span>
          </div>
            ...
            {VALORES.map((v, i) => {
              ...
              return (
                <div key={i} className="flex flex-col items-center text-center gap-3">
                  ...
                  <h3 className="text-xs font-bold text-[#C8A27C] uppercase tracking-wider">{t(v.title)}</h3>
                  <p className="text-[10px] text-gray-400 leading-relaxed">{t(v.desc)}</p>
                </div>
              );
            })}
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {STATS.map((stat, i) => (
              <div key={i} className="flex flex-col items-center text-center gap-1 py-4">
                ...
                <span className="text-[10px] text-gray-400 uppercase tracking-wider font-semibold">{t(stat.label)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 px-4 md:px-8 border-t border-[#3A232E]" style={{ background: '#1A0F14' }}>
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-4">
            <span className="text-xs uppercase tracking-widest text-[#C8A27C] font-bold">{t('NOSSOS PARCEIROS')}</span>
          </div>
          <p className="text-sm text-gray-400 text-center mb-12">{t('Trabalhamos com marcas e profissionais que compartilham dos nossos valores e garantem experiências autênticas, seguras e memoráveis.')}</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {PARCEIROS.map((p, i) => (
              <div key={i} className="flex flex-col items-center justify-center gap-2 p-5 rounded-2xl border border-[#3A232E] hover:border-[#C8A27C]/40 transition-all text-center" style={{ background: '#2A161D' }}>
                ...
                <span className="text-[9px] text-gray-500 uppercase tracking-wider">{t(p.subtitle)}</span>
              </div>
            ))}
          </div>

          <div className="text-center mt-8">
            <Link href={lp('/contato')} className="flex items-center justify-center gap-2 bg-transparent border border-[#3A232E] hover:border-[#C8A27C] text-[#C8A27C] font-bold text-xs py-3 px-8 rounded-xl transition-all uppercase tracking-wider mx-auto w-fit">
      ...
      <section className="py-0 border-t border-[#3A232E]" style={{ background: '#1A0F14' }}>
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2">
          ...
          <div className="p-10 md:p-16 flex flex-col justify-center gap-5">
            <span className="text-xs uppercase tracking-widest text-[#C8A27C] font-bold">{t('VAMOS CRIAR ALGO EXTRAORDINÁRIO JUNTOS?')}</span>
            ...
            <p className="text-xs text-gray-400 leading-relaxed">
              {t('A Maeum Global está sempre aberta a novas parcerias que acrescentem valor às nossas jornadas. Se você deseja promover sua marca, produto ou serviço para um público seleto e altamente engajado, entre em contato conosco.')}
            </p>
            ...
            <div className="flex items-center gap-3 bg-[#2A161D] border border-[#3A232E] rounded-xl p-4">
              ...
              <div>
                <span className="text-[10px] text-gray-500 block">{t('Envie sua proposta para:')}</span>
            ...
            <div className="grid grid-cols-3 gap-4 mt-2">
              {[
                { icon: '📈', label: t('VISIBILIDADE'), desc: t('Exposição para um público seleto e qualificado.') },
                { icon: '🤝', label: t('PARCERIAS REAIS'), desc: t('Trabalhamos juntos para criar experiências únicas.') },
                { icon: '🌐', label: t('CONEXÕES'), desc: t('Fortalecemos culturas, negócios e pessoas.') },
              ].map((item, i) => (
      ...
      <section className="py-6 px-4 md:px-8 border-t border-[#3A232E]" style={{ background: '#1A0F14' }}>
        <div className="max-w-7xl mx-auto text-center flex items-center justify-center gap-3">
          ...
          <span className="text-[10px] text-gray-500 uppercase tracking-widest">{t('MAEUM GLOBAL — SUA HISTÓRIA NA COREIA, DO SEU JEITO, COM EXCELÊNCIA.')}</span>
          <div className="text-[#C8A27C] text-xs">✦</div>
        </div>
      </section>
