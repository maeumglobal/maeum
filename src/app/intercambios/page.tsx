...
export default function IntercambiosPage() {
  ...
  const cardSeul = useMedia('intercambios_card_seul');
  const cardToquio = useMedia('intercambios_card_toquio');

  const intercambios = [
    {
      title: t('Intercâmbio de Idiomas & Cultura em Seul'),
      duration: t('4 a 12 semanas'),
      institution: 'Sogang University / Lexis Korea',
      desc: t('Aprenda coreano no coração de Seul, com hospedagem em estúdio de luxo e atividades culturais inclusas (culinária, caligrafia e K-Pop).'),
      price: t('A partir de US$ 2,800'),
      image: cardSeul
    },
    {
      title: t('Semestre Acadêmico em Tóquio'),
      duration: t('6 meses'),
      institution: 'Waseda University',
      desc: t('Imersão completa no ambiente acadêmico japonês com aulas de negócios e cultura. Acomodação premium em Shinjuku.'),
      price: t('A partir de US$ 5,900'),
      image: cardToquio
    }
  ];
    ...
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <main className="flex-1 py-16 px-4 md:px-8 max-w-6xl mx-auto w-full">
        <div className="max-w-2xl mb-12">
          <span className="text-xs uppercase tracking-widest text-primary font-bold">{t('Imersão de Prestígio')}</span>
          <h1 className="font-heading text-4xl sm:text-5xl font-light text-secondary mt-2">{t('Programas de Intercâmbio')}</h1>
          <p className="text-xs text-muted-foreground mt-3 leading-relaxed">
            {t('Estude nas melhores universidades da Ásia com suporte completo da Maeum Global. Cuidamos de todo o processo de visto, matrícula, hospedagem executiva e acolhimento local.')}
          </p>
        </div>
